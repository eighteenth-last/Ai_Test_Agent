...
### 13. 禅道集成（Zentao Integration）
- **双向数据同步**：与禅道项目管理系统深度集成，支持 Bug 推送和用例双向同步
- **Bug 自动推送**：
  - 一键将本地 Bug 报告推送到禅道，自动填写标题、复现步骤、严重程度、影响模块
  - 支持 v1/v2 双版本 API 自动探测，兼容禅道 15.x ~ 21.x OpenSource
  - HTTP 200 业务级错误检测（`result: fail`），避免静默失败
- **用例双向导入**：
  - 三种导入模式：按产品一键导入全部用例、按测试套件、按用例 ID 精确导入
  - **并发拉取**：使用 `asyncio.Semaphore` 控制并发数（默认 5，上限 20），大批量用例数十秒完成
  - **limit 参数**：可限制单次最多导入数量，适合初次验证
  - **自动去重**：导入前查询本地已有标题，跳过重复；同批次内也防止重复写入
  - **全量字段提取**：逐条调用单用例详情接口，保证 `steps`/`precondition`/`expected`/`keywords` 字段完整写入
- **Bug 管理**：
  - 本地 Bug 报告列表，服务端分页，支持按状态/严重程度筛选、分页大小切换（10/20/50/100）
  - 一键将本地 Bug 同步推送到禅道
- **禅道配置管理**：支持多配置实例，可视化管理 URL、Token、默认产品 ID
- **步骤格式处理工具**（`stepsUtils.js`）：统一处理禅道 `[{step, expected}]` 格式与系统内部字符串数组格式的双向转换，兼容历史数据
...
│   ├── Zentao_manage/          # 禅道集成模块
│   │   ├── router.py           # 禅道配置/Bug推送/用例导入/产品列表 API
│   │   ├── service.py          # 业务逻辑（配置管理/Bug推送响应解析/用例并发导入/去重）
│   │   └── client.py           # 禅道 API 客户端（v1/v2 自适应/Token 管理/业务错误检测）
...
│   │   │       └── PromptList.vue
│   │   │   ├── zentao/         # 禅道集成
│   │   │   │   ├── ZentaoConfig.vue  # 禅道配置管理
│   │   │   │   ├── ZentaoBugs.vue    # Bug 管理（分页/筛选/推送到禅道）
│   │   │   │   └── ZentaoCases.vue   # 用例导入（三种模式/并发参数/limit 参数）
│   │   ├── components/         # 公共组件
│   │   ├── composables/        # 组合式函数
│   │   ├── utils/              # 工具函数
...
### 11. 禅道集成
- 在侧边栏进入「禅道集成」模块，先进入**禅道配置**页面新增并激活一条配置（填写 Base URL、API Token、默认产品 ID）
- **Bug 管理**：
  - 列表展示本地所有 Bug 报告（服务端分页），可按状态/严重程度筛选
  - 点击「推送到禅道」将 Bug 单推送至禅道，支持自动填写模块、类型、严重程度、复现步骤
  - 推送失败时界面会显示禅道返回的具体错误原因
- **用例导入**：
  - 在用例导入页面查看禅道产品列表，点击「导入全部用例」快速触发导入
  - 也可点击「开始导入」按钮，在弹窗中精细配置：
    - 导入方式：按产品 / 按套件 / 按 ID 精确导入
    - **最多导入**：设置 limit 数量（0 = 不限制），适合先小批验证
    - **并发数**：控制同时拉取的详情请求数（默认 5，网络好可调高到 10-15）
  - 导入完成后在「用例管理」页查看导入的用例，步骤/前置条件/预期结果均已完整存储
  - 重复导入会自动跳过已存在的同名用例，不产生重复数据
